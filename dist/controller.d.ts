import { ItemType, ChangeData, Point, BoundRect } from './types';
export declare const nwse = "data:image/png;base64,";
export declare const ns = "data:image/png;base64,";
export declare const fullCircleRadius: number;
export declare const Cursors: {
    l: string;
    lt: string;
    t: string;
    tr: string;
    r: string;
    rb: string;
    b: string;
    lb: string;
    rotate: string;
    skew: string;
    get(dir: ItemType, rotation?: number): Promise<any>;
};
export declare const ItemIcons: {
    rotate: string;
    skew: string;
};
export declare const getRotateEventPosition: (offset: Point, oldPosition: Point, newPosition: Point, rotation: number, center: Point) => Point;
export declare const rotateChange: (oldPosition: Point, newPosition: Point, center: Point) => number;
export declare const getChangeData: (dir: ItemType, offset: Point, oldPosition: Point, newPosition: Point, center: Point, bounds: BoundRect, rotation?: number) => ChangeData;
